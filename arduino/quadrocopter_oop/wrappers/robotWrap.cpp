...
RobotWrapper::RobotWrapper(QThread *guiThread, bool isRobo)
{
    mHasRealRobot = isRobo;//guiThread != nullptr;
	...
	{
        mBrickEmulator = new emulators::BrickEmulator(4);
		mBrick = nullptr;
...
void RobotWrapper::wrapRealDevices()
{
    ...
    mGyro = new GyroscopeWrap(mBrick->gyroscope());
}
...
void RobotWrapper::wrapEmulators()
{
    ...
    mGyro = new GyroscopeWrap(mBrickEmulator->gyroscope());
}
...
GyroscopeWrap *RobotWrapper::gyroscope()
{
    return mGyro;
}
